...
| Introduction and overview - scenario table frame | 10-16 | `scenario/getting-started.md` | reviewed-complete | pdf-checked | 2026-06-08 | Authored from and checked against printed p.10-16: table framing, session setup, who-reads-what spoiler split, investigator types, and setting tone. Mechanical frame is in `00-system-overview.md`; the intro carries no dedicated safety-tools section (table boundaries/safety advice is in `scenario/gm-procedures.md`, Ch 5). Table 1 glossary is already reproduced in `00-system-overview.md` and not duplicated. |
...
| Chapter 6: A Rogues' Gallery - scenario NPC summaries | 231-258 | `scenario/npcs-and-beings.md` | drafted | index-only | 2026-06-02 | Paraphrased GM-facing index (roles + pointers); no profile prose reproduced. Full stat blocks live in `globaldata/npcs/*.json` (verified at NPC extraction); being mechanics in `11-demi-monde.md`. Checked for missing rule mechanics: the general being-type rules (chimeras, High Fae, lower fae, talking foxes, trolls, genii locorum, ghosts, vampires, unicorns, Quiet People, Mystic Art / Wizard's Sword) are already in `11-demi-monde.md` and mirrored in `../../rules-advanced/11-demi-monde.md`; per-being Powers/affinity/Vestigia are stat-block content in the NPC sheets. Nothing new to add. |
| Chapter 7: Welcome to London | 259-284 | `scenario/folly-and-london.md`, `scenario/case-seeds.md` | reviewed-complete | pdf-checked | 2026-06-08 | PDF-gated against printed p.259-284: Folly history/building/rooms, the magical organisations (Society of the Wise/Rose, Little Crocodiles, Paternoster Society, Sons of Weyland, County Practitioners, hedge witches), police facilities, the London gazetteer, the rivers/genii locorum, and the demi-monde pub/nazareth rules all verified. All 12 case seeds present in `scenario/case-seeds.md`. In-text bracketed location numbers cross-checked and consistent (one fix: Skygarden labelled (18) to match the source heading). The printed Map of Central London (p.270-271) is image-only and extracts blank to text; bracket numbers were verified from the body text rather than the map image. Printed p.284 is a chapter-divider with no text. |
...
| Appendix A: Ready-to-Play Investigators | 343-349 | `globaldata/npcs/*.json` (6 pre-gens) | reviewed-complete | pdf-checked | 2026-06-08 | Six pre-gens (Nafeesa Jones, Morgan Omans, Jordan Schneider, Eli Venturini, Jules Garland, Mina Patel) transcribed from printed p.343-349 into unallocated global NPC sheets (`scope: []`). Stat blocks checked against the PDF; backstories paraphrased, not copied. Seed-verified. Portrait restyle from the page crops is a follow-up. |
...
|---|---|---|---|---|
| Scenario-facing introduction and table frame not yet extracted. | `scenario/getting-started.md` | Introduction 10-16 | closed 2026-06-08 | Authored `scenario/getting-started.md` from printed p.10-16 and PDF-gated in the same pass. No dedicated safety-tools section exists in the intro; table boundaries/safety advice is covered in `scenario/gm-procedures.md`. |
...
| Chapter 7 drafted from the source section without the printed-PDF gate. | `scenario/folly-and-london.md`, `scenario/case-seeds.md` | Chapter 7, pages 259-284 | closed 2026-06-08 | PDF-checked both drafts against printed p.259-284: place/organisation/river facts, pub and nazareth rules, and all 12 case seeds verified. Bracketed location numbers cross-checked against the body text and corrected one (Skygarden → (18)). The Map of Central London (p.270-271) is image-only; numbers were sourced from the prose, so no map-image reparse is required for the text gate. Both files promoted to `reviewed-complete`. |
