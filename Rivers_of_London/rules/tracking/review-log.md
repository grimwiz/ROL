...
- Added canonical Bookshop NPC allocation: Warwick Anderson, Saffron Jackson, PC Karnam Singh, Ernie, and the Spirit of Books and Reading are imported from JSON-backed NPC sheets and allocated to the case. Reset ensures the cast is allocated without creating per-case sheet copies.
- Linked existing global NPC sheets referenced by the Bookshop case: DCI Thomas Nightingale, DC Peter Grant, and Toby are now allocated alongside the local case cast.
